...
export default function NewsCard({ news }: NewsCardProps) {
  const hasPublication =
    (news.links && news.links.length > 0) || Boolean(news.paperUrl);

  return (
    <Link
      href={`/news/${news.id}`}
      className="flex flex-col md:flex-row gap-6 border-b border-[#a3a3a3] py-10 group bg-white w-full"
    >
      ...
      <div className="block md:hidden relative w-full aspect-[589/332]">
        ...
        <div className="flex flex-col gap-[10px] md:gap-[19px] leading-[1.1]">
          <div className="flex gap-3 items-center flex-wrap">
            ...
            {news.venue && (
              <span className="font-mono text-xs md:text-sm font-normal uppercase tracking-[0.32px] text-[#001f33] border border-[#001f33] px-[6px] py-[2px]">
                {news.venue}
              </span>
            )}
          </div>
        ...
        {/* Only claim a publication when one actually exists. This tag is not a
            link: the whole card already navigates to the article. */}
        {hasPublication && (
